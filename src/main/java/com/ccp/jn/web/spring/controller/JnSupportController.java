...
public class JnSupportController {
	...
	@Operation(summary = "Criar senha temporária para usuário desbloquear token", description = "Quando ocorre? Quando usuário bloqueia o token e solicita desbloqueio deste token, o sistema envia por mensagem instantânea esta solicitação"
	 ...
	 @ApiResponses(value = {
		        ...
		        @ApiResponse(responseCode = "401", description = "Status: 'Operador sem permissão para atender esta solicitação' <br/><br/> Quando ocorre? Quando o operador não está cadastrado na tabela correta para atender este tipo de solicitação."),
		        @ApiResponse(responseCode = "404", description = "Status: 'Token não encontrado' <br/><br/> Quando ocorre? Quando o token do usuário não é encontrado."),
		        @ApiResponse(responseCode = "409", description = "Status: 'Solicitação já respondida' <br/><br/> Quando ocorre? Quando a solicitação consta na tabela de solicitações (deste tipo) respondidas."),
		        @ApiResponse(responseCode = "422", description = "Status: 'Solicitação inexistente' <br/><br/> Quando ocorre? Quando a solicitação não existe."),
	 })	
